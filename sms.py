...
def toplevel_data(title,button_text,command):
    global idEntry,phoneEntry,nameEntry,emailEntry,addressEntry,genderEntry,dobEntry,screen_window
    ...
    idEntry=Entry(screen_window,font=('roman',15,'bold'))
    ...
    phoneLabel=Label(screen_window,text='Phone',font=('times new roman',20,'bold'))
    phoneLabel.grid(row=2,column=0,padx=30,pady=15,sticky=W)
    phoneEntry=Entry(screen_window,font=('roman',15,'bold'),width=24)
...
def search_data():
    query = 'select * from Students where id=%s or name=%s or email=%s or mobile=%s or address=%s or gender=%s or dob=%s,date=%s,time=%s where id=%s'
    mycursor.execute(query, (idEntry.get(), nameEntry.get(), emailEntry.get(), phoneEntry.get(),addressEntry.get(), genderEntry.get(), dobEntry.get(),date,currenttime,idEntry.get()))
    con.commit()
    messagebox.showinfo('Success',f'Id {idEntry.get()} is modified successfully',parent=screen_window)
    screen_window.destroy()
    show_student() 
...
def add_data():
    if idEntry.get() == '' or nameEntry.get() == '' or phoneEntry.get() == '' or emailEntry.get() == '' or addressEntry.get() == '' or genderEntry.get() == '' or dobEntry.get() == '':
        ...
        try:
            ...
            con.commit()
            result=messagebox.askyesno('Confirm','Data added successfully.Do you want to clean the form?',parent=screen_window)
...
def connect_database():
    def connect():
        ...
        try:
            ...
            mycursor.execute(query)
            query='create table Students(id int not null primary key, name varchar(30),mobile varchar(10),email varchar(30),address varchar(100),gender varchar(20),dob varchar(20),date varchar(50),time varchar(50))'
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
...
sliderLabel=Label(root,text='STUDENT MANAGEMENT SYSTEM',font=('arial',20,'italic bold'))
sliderLabel.place(x=200,y=0)
connectButton=Button(root,text='connect data base',bg='orange',command=connect_database)
connectButton.place(x=980,y=0)

leftFrame=Frame(root)
leftFrame.place(x=30,y=80,width=400,height=500,)

addstudentButton=Button(leftFrame,text='Add student',font=('times new roman',10),bg='orange',width=25,state=DISABLED,command=lambda:toplevel_data('Add Student','Add',add_data))
addstudentButton.grid(row=1,column=0,pady=20)

searchstudentButton=Button(leftFrame,text='Search student',font=('times new roman',10),bg='orange',width=25,state=DISABLED,command=lambda:toplevel_data('Search Student','Search',search_data))
searchstudentButton.grid(row=2,column=0,pady=20)

deletestudentButton=Button(leftFrame,text='Delete student',font=('times new roman',10),bg='orange',width=25,state=DISABLED,command=delete_student)
deletestudentButton.grid(row=3,column=0,pady=20)

updatestudentButton=Button(leftFrame,text='Update student',font=('times new roman',10),bg='orange',width=25,state=DISABLED,command=lambda:toplevel_data('Update Student','Update',update_data))
updatestudentButton.grid(row=4,column=0,pady=20)

showstudentButton=Button(leftFrame,text='Show student',font=('times new roman',10),bg='orange',width=25,state=DISABLED,command=show_student)
showstudentButton.grid(row=5,column=0,pady=20)

exportstudentButton=Button(leftFrame,text='Export data',font=('times new roman',10),bg='orange',width=25,state=DISABLED,command=export_data)
exportstudentButton.grid(row=6,column=0,pady=20)

exitButton=Button(leftFrame,text='Exit',font=('times new roman',10),bg='orange',width=25,command=iexit)
...
style.configure('Treeview',font=('arial',14,'bold'),foreground='red')
